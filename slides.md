...
- 最初に作った Unity AI 作例。
- 当時は Unity Lite モデルしか存在せず、それを使用した。
- C# スクリプト、シェーダー、スプライト、アニメーション（スプライトシート）、効果音、等々、全てのアセットを Unity AI で生成。
- Unity Editor 上の操作もなるべく AI Assistant に任せた。
- ２週間で制作

<!--
このプロジェクトの開発背景を少し解説しておきたいと思います。これは個人的に初めて作った Unity AI の作例でした。当時はまだ Unity Lite モデルしか存在していなかったため、すべての作業を Unity Lite モデルを使って行なっています。

プロジェクト内で使用しているアセットは基本的にすべて Unity AI を使って作成しています。C# スクリプトやシェーダーは AI Assistant で作成していますし、スプライトや、それをアニメーションさせるためのスプライトシート、効果音などはすべて Generator で作成しました。

また、AI Assistant にどこまで作業を任せられるか知りたかったので、Unity Editor 自体の操作もほとんど AI Assistant に任せるようにしました。結果的に、自分で Hierarchy をクリックしてポチポチと作業することは殆どありませんでした。

これは特に締め切りのないプロジェクトだったので、ズルズルと制作を続けたのですが、だいたい２週間でこのような形になりました。
-->
...
<!--
次にこのプロジェクトの開発の流れについて解説していきます。

ただ、まだこの時点では Unity AI を使い慣れていなかったので、あまり参考になる情報は提供できないと思います。Unity AI の実際の詳しい使い方については後のプロジェクトに譲るとして、ここでは要点だけを挙げてサラッと流したいと思います。
-->
...
まず最初に作成したプロトタイプは、こんな感じでした。全然違うゲームであったのが分かると思います。「敵の弾を反射して利用する」というコアのメカニクスは同じなのですが、最初は SF 風テーマの 2D シューティングゲームとして作っていたのが分かると思います。
...
このプロトタイプは、こんな感じのプロンプトで、ほぼ one shot で作成しました。Unite Lite でもこの程度のタスクは難なくこなせる感じですね。

#### 大幅な方針転換

- シューティングゲームとしての伸び代が感じられず、どう発展させるべきか見えなくなってきた。
- そこで思い切って、いわゆる "Survivors" 系の方向性を組み込んでみることにした。
- その結果、多数の改善が得られた。
  - わかりやすいキャラクター
  - 防御しかできないことの物語的必然性
  - RPG 的な成長要素としてのパワーアップ
  - 既存ジャンルからの書式の取り込み

ただ、このプロトタイプを実際に作ってみて感じたのですが、シューティングゲームとしての伸び代が感じられず、この先どう発展させていくべきか分からなくなってきました。

そこで、ここで大幅な方針転換を行うことにしました。古典的なシューティングゲームではなく、いわゆる "Survivors" 系のゲームとして作り替えてみよう、と考えたわけです。

この方針転換によって大きく道がひらけました。顔の無い宇宙船ではなく、わかりやすい、人格の見えるキャラクターになり、またそこから、防御しかできないことの物語的必然性が生まれました。パワーアップ要素は RPG 的な成長要素として説明することができるようになりましたし、スキルツリーなどの要素を今後追加していくことも可能だと思います。

重要なのは、このような大きな方針転換というのは、従来であれば精神的負荷の高い大変な作業であったのが、AI assisted なゲーム開発であれば、比較的容易に行えるということです。特に、今回のこの事例のようなコンテキストの小さな初期プロトタイプの段階であれば、容易に転換が可能ですし、あるいは最初から作り直しを何度も行うというアプローチでもいいかもしれません。

この「消耗せずに何度でも作り直せる」というのは、AI assisted なゲーム開発の明確なメリットの一つだと思いますし、僕自身もそのメリットの大きさを実感することになりました。

### 細かなテクニック

次に、このプロジェクトで使用した、ちょっと細かなテクニックについて触れたいと思います。
...
このエフェクト自体は、僕が過去に開発した KinoEight というパッケージをそのまま使用しています。

#### エフェクトを入れた理由

- 単にレトロゲーム風にしたかったわけではない。
- 「アセットの持つ情報量」の均一化が狙い。
  - AI 生成アセットでも、購入して集めたアセットでも、それぞれの「情報量」（書き込みの密度や色彩の深さなど）にばらつきが生じやすい。
  - 統合感の無さを感じさせる要因になる。
  - これを均一化するためのテクニックとして、「一旦全てのアセットの情報量を削って落とす」と言うものがある。
  - レトロ風のエフェクトはこれを実現できる。
...
この KinoEight は、解像度と色彩を強制的に落とすことによって、そのような効果を生み出すことができます。

これは、画像を劣化させることで見た目を揃える、ということなので、実はあまり使いたくないテクニックなのですが、今回は Unity AI の Generator を使うのが初めてだったので、安直なテクニックに頼ってみた、という格好です。
...
<!--
次に、２つ目に取り組んだプロジェクト、Dungeon Match Heroes について解説したいと思います。
-->
...
- 経験値を貯めてレベルアップ。積極的にレベルアップを狙わないと、どんどん難易度が上がっていってしまう。
...
RPG 的な成長要素があり、経験値を貯めることでレベルアップします。経験値は敵を倒すだけでなく、鍵ブロックを使って宝箱を開けたり、多くのブロックをいっぺんに消したりすることで稼げます。敵は徐々に強くなっていくので、そうやってうまく経験値を稼いでいかないと、段々とプレイが難しくなっていく、という内容になっています。
-->
...
- Unity AI を使って、プロトタイプ以上のもの、ゲームとして必要な要素が、必要なクオリティで、一通りちゃんと入ったものが作れるのか？と言う疑問に答えるために作成した。
- そのため、ゲーム内容としては無難なもの、既にありそうなものを敢えて選んだ。

<!--
開発背景についてですが、前回の Mirror Mage の開発を通して Unity AI の使い方は大体分かってきたので、今回のプロジェクトでは、もう少し本格的にゲームとして作り込んでみようと考えました。規模として大きなものを作る余裕はありませんでしたが、バーティカル・スライスとして、規模を限定しながらも完成クオリティのものを目指す、という方針です。

その目的のため、ゲーム内容としては実験的なものではなく、既にありそうなものを敢えて選びました。実験が目的ではなく、我々の日常の業務がちゃんとできるかどうか確かめたかった、ということですね。
-->
...
<!--
まだこの段階でも、Unity Lite モデルしか存在していませんでしたので、Unity Lite を全ての作業に使用しました。また、各種のアセットも Generator で生成し、Unity Editor の操作もなるべく AI Assistant 任せにしました。この辺りは前作の Mirror Mage と同様ですね。
-->
...
その他に、このプロジェクトでは MCP Client 機能も使用しました。MCP Server ではなく、MCP Client の方ですね。つまり、Unity 内の AI Assistant から外部のソフトウェアを制御する、というものです。

ここでは、3D モデルを、いわゆるマニフォールドモデルとかソリッドモデルとか言われる「切断可能なモデル」に変換するための加工を Blender に行ってもらうことにしました。

3D モデルをマニフォールドモデルに変換するには、Blender の機能をいくつか順番に呼び出していく必要があるのですが、この作業を AI Assistant に行ってもらったわけですね。

#### スキルの使用

- スキル設定の図

また、その「Blender の機能を順番に呼び出す」という手続きを AI エージェントが利用可能なスキルの形にまとめて、プロジェクトスキルとして登録しました。

このような MCP Client 機能とスキル機能の組み合わせによって、マニフォールドモデル変換が完全に自動化されました。AI Assistant に「このモデルをマニフォールド化して」と命令すれば、あとは勝手に Blender を呼び出して変換してくれるようになったわけですね。
