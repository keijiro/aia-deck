...
<div class="absolute inset-0 flex items-center justify-center" style="color:#39AAFF">
  <div class="flex flex-col items-center text-center border-2 px-16 py-12" style="border-color:#39AAFF">
    <div class="text-3xl font-extrabold" style="line-height:1.5">
      音声付き動画の再生を許可するため、<br>メインスクリーンをクリックしてください。
    </div>
  </div>
</div>
...
本日のこのセッションでは、基本的には Unity AI の話をする予定なんですが、その前にちょっと時間を割きまして
-->
...
<!--
今週はじめ、というか、つい一昨日の話ですね、韓国で開催された Unite Seoul での発表を少しリキャップしたいと思います。というのも、Unity の AI 機能に関連するいくつかの重要がアップデートがあったんですよね。これを語らずして Unity AI を語るわけにはいかない、というぐらい重要なものなので、今日のセッションの内容を予定と若干変更することにはなりますが、解説させていただきたいと思います。
-->
...
<img src="/roadmap1.png" class="absolute inset-0 w-full h-full object-cover" />

<!--
まず、一つ目、大きな所としては、AI 関連の機能がデスクトップアプリとして外部化されて新登場、というものです。名前もちょっと変わっていて、Agentic Authoring tool というものになっていますね。これはまあ分かりやすく解説すると、今まで Unity Editor に内蔵されていた AI Assistant と Generator が、外部のデスクトップアプリ化される、というものです。機能的には AI Asssitant のものをほぼそのまま踏襲していて、それにプラスαがある、って感じですね。既に AI Assistant を使っていた方は、何の違和感もなく移行できると思います。

それでいて、このようなエージェンティックなオーサリングツールが外部化されることによって嬉しいことがいくつかあります。最も大きな所としては、C# スクリプトを更新した際の domain reload の影響を受けなくなるという点ですね。今までは、AI Assistant でプロンプトを打って何かスクリプトが更新されると、その度に domain reload が走って、その間は AI Assistant が無反応になってしまう、という弱点がありました。それが、このように外部化されたことによって、domain reload による束縛が無くなって、バリバリ C# スクリプトを書き続けられるようになったわけですね。

実際に今日紹介するサンプルプロジェクトの中にも、実はこのエージェンティック・オーサリング機能を先行して使用して作成したものがあります。実際にかなり使いやすいツールになっていることが確認できました。

ということで、現在 Unity Editor 上の AI Assistant を使用している方については、こちらの agentic authoring tool への移行を今後はお勧めすることになると思います。近日中に closed beta テストを始めて、その後 open beta を開始するとのことですので、興味のある方はぜひそちらで試してみていただきたいです。
-->
...
<img src="/roadmap2.png" class="absolute inset-0 w-full h-full object-cover" />

<!--
そしてもう一つの重要なアップデート、こちらは更に重要なアップデートになりますが、Unity production pipeline および Unity CLI ですね。
-->
...
<img src="/roadmap3.png" class="absolute inset-0 w-full h-full object-cover" />

<!--
これは単一の機能ではなく、さまざまな機能の総称になるのですが、簡単にまとめると、今まで Unity プロジェクトを Unity Editor からしかアクセスすることができなかったのを、Editor 以外の経路からもアクセスできるようにしていこう、という取り組みですね。

それはウェブ上のダッシュボードであったり、REST API 経由でのアクセスであったり、AI Agent からのアクセスであったり、色々あるんですが、その中でも特に重要視しているのが
-->
...
<img src="/roadmap4.png" class="absolute inset-0 w-full h-full object-cover" />

<!--
CLI 経由でのアクセスですね。これがいわゆる Unity CLI と呼ばれるものです。
...
<!--
この Unity CLI、これはまんま unity って名前のコマンドなんですけれど、基本としては、Unity にまつわる様々なことをコマンドラインから行えるようにしたものです。例えば、unity install で Unity Editor をインストールしたり……これ通称が使えるので、例えば unity install 6.3 とかすれば、6000.3 の最も新しいバージョンをインストールしてくれたりしますね。そのあと unity install-modules で iOS サポートをインストールしたりとか、unity open . でカレントディレクトリにあるプロジェクトを開いたりとか、そう言うのですね。今まで Unity Hub やら何やら使っていたことは、基本的に全て unity コマンドからできます。

ええと、automated オプションを使った方がいいと出ているので、ちょっと起動しなおしましょうかね。

<pre>
unity open . --args -automated
</pre>

これは Unity Editor の警告ダイアログなどを抑制するためのオプションなんですが、コーディングエージェントなどから CLI 経由で制御する場合にモーダルダイアログが表示されて処理が止まってしまうと困るので、これで抑制ができるんですね。Batch Mode に似ているんですけど、Batch Mode よりも普段の Editor の挙動に近い感じになります。

で、この Unity CLI の本領発揮となるのが、Unity pipeline 連携ですね。Unity プロジェクト側に Unity Pipeline パッケージをインストールしておくと……これは unity pipeline install で実行できるんですけど、そうすると、pipeline 連携を使えるようになります。ちょっと接続状態を確認しましょう。

<pre>
unity pipeline list
</pre>

大丈夫ですね。pipeline が接続されると、プロジェクト側に用意されているコマンドを様々に呼び出せるようになります。コマンドのリストアップは cmd でいけます。

<pre>
unity cmd
</pre>

ビルドインのコマンドだけでこんなにあるんですけど……最近むっちゃ増えて 140 個になりましたね。ちょっと検索できるようにして、

<pre>
unity cmd | nvim -
</pre>

例えば、editor_play で play mode に入ったり、create_scene でシーンを作ったり、screenshot でスクショを撮ったり、run_test でテストを実行したり、editor_focus でエディタを前面に出したり……まあ Unity プロジェクトに関する基本的な操作は、これで何でもできるという感じです。試しにやってみましょうか。

<pre>
unity cmd editor_play
</pre>

このコマンドはカスタムで何でも登録できるので、ゲームに関する機能もガンガン登録していくことができます。

もっとも、このビルトインコマンドの中には eval っていう、C# コードを何でも実行する機能もあるので、目的とするコマンドがあろうがなかろうが、最終的には C# コードで何とかなってしまうと言う感じでもあります。ちょっとやってみましょうか。これはちょっと長いのでカンペからコピペします。ゲームオブジェクトのリストを取得するコードですね。

<pre>
unity cmd eval --code 'return UnityEngine.Object.FindObjectsByType&lt;GameObject&gt;(UnityEngine.FindObjectsSortMode.None).ToDictionary(o=>o.GetInstanceID(),o=>o.name);'
</pre>

これで取得できました。この eval は pipeline パッケージ側に Roslyn コンパイラがあって、オンザフライでコンパイルしたアセンブリでコードを動かすので、ドメインリロードなどは発生しません。高速に任意のコードを動かすことができます。

それで、この eval コマンドは、development build でも使用することができます。実際にやってみましょう。まずエディタを閉じてしまって、あらかじめビルドしておいたものを実行します。

<pre>
open Builds/Player.app
</pre>

そして、ランタイムと繋ぎたいので、 <code>--runtime-path Builds/Player.app</code> を加えましょう。はい、これで同じようにビルドされたプレイヤーからも取得できましたね。

副作用のある処理もできます。これも長いので、カンペからコピペしようかな。

<pre>
unity cmd eval --code 'UnityEngine.Object.Destroy(UnityEngine.GameObject.Find("UnityMaterialBall"));' --runtime-path Builds/Player.app
</pre>

こんな感じで色々と便利な eval コマンドなんですが、更に強力な機能としてホットリロード機能も用意されています。Unite で弊社の Will さんがデモをしていたので、その映像をちょっと持ってきたので再生しましょうか。
-->
...
<SlidevVideo autoplay autoreset="slide" :muted="$renderContext !== 'slide'" class="absolute inset-0 w-full h-full object-cover" print-timestamp="3" >
  <source src="/willg-demo-short-hq.mp4" type="video/mp4" />
</SlidevVideo>

---

<!--
こんな感じで、実行中のビルドのコードを入れ替えることができるんですね。ちょっと実際にやってみましょうか。まず、再びビルドを実行して、

<pre>
open Builds/Player.app
</pre>

この色を設定しているコードが、ここにあります。

<pre>
nvim Assets/ColorChanger.cs
</pre>

ホットリロード可能にするために、<code>[HotReload]</code> 属性をあらかじめつけてあります。色を適当に変更して、リロードを実行しましょう。これもちょっと長いのでコピペして、

<pre>
unity cmd reload_file --runtime-path Builds/Player.app --filename ../Assets/ColorChanger.cs
</pre>

はい、これで適用されました。こんな感じです。

このホットリロード機能は、現状では Editor 上と、デスクトッププラットフォーム向けの standalong ビルドでのみ使用できます。その他のプラットフォームのビルドについては今後対応予定です。

先ほどの Will さんのデモでは iPhone 実機で動いていたので、あれっ、と思うかもしれませんが、あれは開発中の機能です。開発ブランチでは実際に動いているんですけど……まあ皆さん恐らくご想像の通り、かなり凝ったことをやっているので、安定して使えるようになるまでもう少しお待ちください、という感じです。

-->

---

<img src="/cli-demo1.png" class="absolute inset-0 w-full h-full object-cover" />

<!--
と、こんな風に様々な機能を提供する、というか、Unity プロジェクトのオーサリングについて新しい手段を様々に提供する、Unity CLI および Pipeline パッケージなのですが、重要なのは、これが CLI であるが故に、コーディングエージェントとの親和性が非常に高いということですね。今のデモンストレーションは全て手作業でやりましたが、同じくことを Claude Code や Codex のようなコーディングエージェントにやってもらうことも可能です。

コーディングエージェントが、Unity のバージョンをインストールして、プロジェクトを作って、スクリプトをコンパイルして、シーンを構築して、Play Mode で動かして、スクショを撮って状態を確認して、テストを書いたり実行したりして、ビルドしてそれを実行して、実行しながら eval やホットリロードを使ってリアルタイムに調整して……みたいなことを、全部エージェントにやってもらうことが可能だというのは、もう簡単に想像できますよね。

先日の Unite では MCP Server を無料化します、という発表を行って、現に昨日から無料化されているんですけど、実際にはエージェントとの連携については Unity CLI の方で全てカバーできてしまうので、まずは Unity CLI を試してみていただくのを、今はお勧めしています。

インストールの方法は、まあググると見つかるんですが、個人的にお勧めなのは、Unity 公式スキルを先にインストールしてしまうことです。こちらに Unity 公式のコーディングエージェント用スキルリポジトリがあります。

https://github.com/Unity-Technologies/skills

ここに様々なスキルが含まれているんですが、当然 Unity CLI も存在していて、インストール方法もここに記載されています。つまりこのスキルをインストールすれば、エージェントに「最新版の Unity CLI をインストールして」って頼めばインストールしてくれますし、Unity CLI の使い方についてもバッチリ把握した状態になっているということですね。
-->
...
<!--
と言った感じで Unite Seoul のリキャップは以上になります。

そろそろいい加減に本題に入らないと怒られてしまうので、本編を開始しましょう。
-->
...
- 防御魔法しか使えない魔法使いを主人公とした、いわゆる "Survivors" 系ゲーム。
...
- MCP Tools 及びスキル機能は外部ツールとの連携に便利
...
<img src="/qr-code.png" class="absolute" style="left:54%; top:26.8%; width:26%; height:46.2%" />
